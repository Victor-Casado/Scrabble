...
pub fn leave_team(ctx: &ReducerContext) -> Result<(), String> {
    ...
    if remaining == 0 {
        ...
        for bid in bot_ids {
            ...
            if ctx.db.bot_stats().bot_id().find(bid).is_some() {
                ctx.db.bot_stats().bot_id().delete(bid);
            }
...
#[reducer]
pub fn cleanup_orphan_bot_stats(ctx: &ReducerContext) -> Result<(), String> {
    let bot_ids: Vec<u64> = ctx
        .db
        .bot_stats()
        .iter()
        .filter(|stats| ctx.db.bot().id().find(stats.bot_id).is_none())
        .map(|stats| stats.bot_id)
        .collect();
    let count = bot_ids.len();

    for bot_id in bot_ids {
        ctx.db.bot_stats().bot_id().delete(bot_id);
    }
    if count > 0 {
        log::info!("Cleaned up {} orphan bot_stats rows", count);
    }
